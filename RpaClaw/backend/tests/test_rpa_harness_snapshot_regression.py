...
def test_snapshot_regression_fails_when_region_scope_is_lost(tmp_path: Path):
    assets = _write_checkpoint_asset(tmp_path)
    expected_path = tmp_path / "asset-1" / "steps" / "001" / "expected.json"
    expected_path.write_text(
        json.dumps(
            {
                "snapshot_signals": {
                    "must_preserve_region_scope": {
                        "region_id": "region-1",
    ...
    report = run_snapshot_regression(
        assets,
        snapshot_builder=lambda html, checkpoint: {
            "html": html,
            "region_scope": {
                "region_id": "region-1",
                "mode": "region_scoped_snapshot",
    ...
    assert report["summary"]["failed"] == 1
    assert item["status"] == "failed"
    assert item["failure_category"] == "compact-snapshot-lost-region-scope"
    assert item["region_scope_status"] == "missing"
