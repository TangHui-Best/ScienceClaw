...
def test_region_scoped_trace_expected_signals_preserve_selected_region_semantics():
    draft = build_expected_signal_draft(
        step_intent="Extract the selected table row",
        recording_mode="natural_language",
        trace_events=[
            {
                "trace_type": "ai_operation",
    ...
    assert draft.snapshot_signals["must_preserve_region_scope"] == {
        "region_id": "region-1",
        "mode": "region_scoped_snapshot",
        "frame_path": ["iframe.detail"],
        "inferred_kind": "table_region",
