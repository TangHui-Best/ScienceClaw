...
const STEP_TITLES: Record<RpaFlowStepId, string> = {
  record: '录制', configure: '配置', test: '测试保存',
};
const STEP_CAPTIONS: Record<RpaFlowStepId, string> = {
  record: '采集操作', configure: '修复与参数', test: '回放并保存',
};
const ORDER: RpaFlowStepId[] = ['record', 'configure', 'test'];

const baseStep = (id: RpaFlowStepId, index: number): RpaFlowStepView => ({
  id, index, title: STEP_TITLES[id], caption: STEP_CAPTIONS[id], status: 'disabled',
  disabled: true, disabledReason: '', destructive: false,
});
...
export function buildRpaFlowSteps(state: RpaFlowGuideState): RpaFlowStepView[] {
  ...
  return ORDER.map((id, index) => {
    ...
    if (id === state.currentStep) {
      step.status = 'active'; step.disabled = false; return step;
    }
    if (id === 'record') {
      step.status = 'available'; step.disabled = false;
      step.destructive = state.currentStep !== 'record'; return step;
    }
    if (id === 'configure') {
      if (hasSession) {
        step.status = state.currentStep === 'test' ? 'completed' : 'available';
        step.disabled = false;
      } else step.disabledReason = '等待录制会话';
      return step;
    }
    if (state.currentStep === 'record') { step.disabledReason = '先完成配置'; return step; }
    if (!hasSession) { step.disabledReason = '等待录制会话'; return step; }
    if (diagnostics > 0) { step.disabledReason = `${diagnostics} 个步骤待修复`; return step; }
    step.status = 'available'; step.disabled = false; return step;
  });
}
