...
本次收敛的最终验收不是“新增 trace 优先入口”或“继续兼容旧事实源”，而是彻底移除 `session.steps`、`session.recorded_actions`、`recording_diagnostics`、`legacy_steps` metadata 和 step-index API 作为 RPA 业务状态、前后端接口、生成/测试/保存/MCP 输入的存在。迁移过程中可以短期保留私有输入 DTO 或参考测试 fixture，但它们不能继续出现在新 session 的 accepted timeline、接口契约或 skill metadata 中。
...
- 前后端接口不得以 `step`、`recorded_actions` 或 `legacy_steps` 作为新路径字段；如果短期需要兼容旧开发数据，必须隔离为明确的 legacy helper，并在 removal gate 前删除。
...
- 新建 session 的后端序列化响应不包含 `steps`、`recorded_actions`、`recording_diagnostics`。
- Recorder、Configure、Test、MCP 编辑相关前端代码不读取 `session.steps`，不 merge `recorded_actions`，不调用 `/step/{index}`。
- generate/test/save/MCP/export 只读取 `session.traces`、`trace_diagnostics`、`runtime_results`。
- 保存后的 Skill metadata 不包含 `legacy_steps` 或 `recorded_actions`。
- 测试失败定位返回 `failed_trace_id` / `failed_trace_index`；不再返回或消费 `failed_step_index`。
- 删除与 locator promotion 只接受 `trace_id` 或 `diagnostic_id`。

## Harness And Review Requirements

本迁移是核心架构改造，不允许作为单 agent 的随手重构推进。

- 本分支目标是事实源收敛，不做 Harness observability UI、指标、面板或事件流功能。
- Start Gate 级别为 high-risk；本设计和对应 implementation plan 是 durable Vision Anchor。
- 必须使用 subagent-driven development：每个实现切片由 implementer subagent 完成，并经过 spec-compliance reviewer 与 code-quality reviewer 两级评审。
- 不允许多个 implementer 同时修改同一批核心文件；`manager.py`、`route/rpa.py`、前端 RPA 页面、MCP/export/compiler cleanup 应按接口边界顺序推进。
- 每个实现切片必须先写失败测试，再实现，再记录验证命令与结果。
- 最终 readiness 必须通过 Vision Gate Exit、Readiness Dashboard、Knowledge Capture 和 exhaustive Evidence。
- 最终 Evidence 至少包含：后端目标测试、前端 type-check/build、trace-only removal grep、手动录制到保存 smoke、subagent review 记录。
- 如果任何旧源只能暂时保留，必须标记为 `legacy_*` 私有 helper，且不能出现在新路径接口、metadata、accepted timeline 或最终 removal gate 中。
