...
F024.3-F024.5 又补齐了同一原则在其它层的表现：Full SOP Harness capture
不能让同一输入动作变成两条 accepted fill；URL navigation trace 不能只有开启
Harness 时才同步产生；前端实时和轮询 timeline 都只能展示 Core trace 已有的
`signals.download`，不能从 Harness artifact 合成事实。
...
推广规则：如果开启/关闭 Harness capture 会改变 accepted trace、navigation trace、
download signal、fill 步骤、生成 Skill 或 timeline 语义，优先修 Core 事实捕获、
排序、归并或展示投影边界；不要让 Harness expected signals、controlled fixture、
asset report 或前端补丁成为第二事实源。
