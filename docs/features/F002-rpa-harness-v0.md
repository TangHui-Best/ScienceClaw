...
| F002.10 | 2026-06-01 | pending | Internal Full SOP assets recorded `Ctrl+V` account/password entry as `press V`, so review and generated Skill could not see replayable `fill` steps. | Paste was treated as keyboard transport: uppercase `V` was not filtered from press capture, and the screencast paste channel inserted text without asking the page recorder to persist the focused editable value as a fill. | Filter paste shortcuts case-insensitively, emit paste-on-editable as `fill` with `source_method=paste`, and have screencast paste call the recorder's current editable fill helper after `Input.insertText`. | completed |
| F002.11 | 2026-06-02 | `6101c909` | New sanitized Full SOP assets could show generic input placeholders such as `{{input:input}}`, making review packets and replay configuration harder for humans and agents to understand. | The fill input key derivation stripped non-ASCII field labels and did not recurse through weak `nth` text-box locators, so semantic labels such as `W3账号` or `密码` could collapse to `w3` or `input`. | Add a deterministic Harness input-contract naming helper used only by fill parameterization and cumulative sanitization; preserve non-fill traces, download signals, recorder capture, and compiler action branches. | completed |
| F002.12 | 2026-06-02 | `eab818eb` | PR #59 review found that short fill values such as `a` could corrupt Harness trace metadata and captured HTML, e.g. `<label>` becoming `<l{{input:name}}bel>`. | Fill parameterization reused a global raw-string replacement map for trace nodes, `step_intent`, and entire HTML documents instead of only replacing structured input value positions. | Parameterize only `fill.value` and `input_contract` in trace events; replace recorded values in human text with ASCII token boundaries; sanitize captured HTML only at `input[value]` and `textarea` values. | completed |
| F002.13 | 2026-06-02 | `92132c2e` | Internal Agent core-chain diagnostics wrote `core-chain` reports and generated `skill.py` files into the Agent CLI/project working directory, separating evidence from the asset it described. | Generated Skill export was an ad hoc Agent wrapper concern rather than a canonical Harness asset-local artifact. | Add `run_asset_core_chain` to write `core-chain-report.md`, `core-chain-full-report.json`, and `generated_skills/` under each selected asset directory while reusing existing stateful SOP and step Skill compilation logic. | completed |

## Patch Churn Review

F002 had multiple follow-up slices because the project first built runtime Harness capability, then recovered missing Harness closeout, then used self-bootstrap assets to expose asset-quality gaps. The 2026-06-01 fill-capture follow-ups exposed three additional invariants: Full SOP assets must be persisted from the accepted, sorted trace timeline rather than async browser event arrival order; every persisted review surface must share the same input sanitization contract; and physical input transports such as paste shortcuts must normalize to semantic `fill` facts when an editable value changes. The 2026-06-02 input-contract naming slice adds a fourth boundary: readability improvements belong in fill-only sanitization metadata, not in recorder event capture, accepted trace ordering, compiler action dispatch, or download side-effect merging. F002.12 adds a fifth boundary: sanitization maps must not become global string rewrite rules; trace, human text, and captured HTML each need their own narrow replacement surface. F002.13 adds a sixth boundary: generated Skill and core-chain diagnostic reports are asset execution evidence, so the canonical Harness path writes them under the owning asset directory instead of the Agent working directory. The repeated patches converged on those invariants rather than adding page-specific rules: captured assets must be factual, quality-labeled, order-stable, sanitized, readable, asset-local, and usable by offline regression runners. No GitHub-specific, login-specific, selector-specific, download-specific, or business-flow-specific capture behavior was added.
