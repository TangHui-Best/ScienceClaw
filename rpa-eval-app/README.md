...
手动 region-scoped section text fixture 位于 `/section-texts`。该页面提供三类非 GitHub 通用 DOM 形态：heading + 同级正文、只有 after-context heading 的正文、以及复杂嵌套 container 正文，用于验证 F011 的“可靠 anchor 才确定性编译，缺 anchor 才 runtime AI fallback”边界。
